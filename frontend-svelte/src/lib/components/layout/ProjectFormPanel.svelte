...
>
  ...
  <div class="flex items-center justify-between">
    <div>
      <p class="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-muted-foreground">{i18n.t('projects')}</p>
      <h3 class="text-base font-semibold text-gray-900 dark:text-foreground">{i18n.t('projectManagement')}</h3>
    </div>
    <button
      type="button"
      onclick={onClose}
  ...
  <div class="mt-4 flex-1 min-h-0 space-y-4 overflow-y-auto pb-4 pr-1">
    ...
    >
      <span class="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-muted-foreground">{i18n.t('projectView')}</span>
      <span class="flex items-center gap-1 text-xs font-semibold text-teal-600 dark:text-teal-300">
        {i18n.t('open')} <ExternalLink class="h-3 w-3" />
      </span>
    </button>
    ...
    <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700 dark:border-border dark:bg-card dark:text-foreground">
      <div class="flex items-center justify-between">
        ...
        >
          {showNewProject ? i18n.t('cancel') : i18n.t('new')}
        </button>
      </div>
      ...
      {#if showNewProject}
        <div class="mt-3 space-y-2 rounded-xl border border-gray-200 bg-white p-3 dark:border-border dark:bg-background">
          ...
          <div class="flex gap-2">
            ...
            >
              {creatingProject ? i18n.t('loading') : i18n.t('create')}
            </button>
            <button
              type="button"
      ...
      <div class="mt-3 space-y-1">
        ...
        {:else}
          ...
          {#if projects.length === 0}
            <p class="px-3 py-2 text-xs text-gray-500 dark:text-muted-foreground">{i18n.t('noProjects')}</p>
          {/if}
        {/if}
      </div>
    </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-gray-50 p-4 text-sm text-gray-700 dark:border-border dark:bg-card dark:text-foreground">
      <div class="flex items-center justify-between">
        <p class="text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-muted-foreground">{i18n.t('conversations')}</p>
        <button
          type="button"
      ...
      <div class="mt-3 space-y-1">
        ...
        {:else if threads.length === 0}
          <p class="text-xs text-gray-500 dark:text-muted-foreground">{i18n.t('noConversationsYet')}</p>
        {:else}
